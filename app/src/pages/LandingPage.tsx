  ...
  return (
    <div className="space-y-10">
      <section className="space-y-5 text-center">
        ...
        <p className="mx-auto max-w-2xl text-base leading-7 text-slate">
          A portable, verifiable on-chain identity primitive for builders — connecting identity,
          skills, projects and contribution proofs into an ecosystem-readable builder profile.
        </p>
        ...
        {totalSupplyQuery.data !== undefined && totalSupplyQuery.data > 0n && (
          <p className="font-mono text-xs text-slate">
            {totalSupplyQuery.data.toString()} builder{totalSupplyQuery.data === 1n ? "" : "s"} minted so far
          </p>
        )}
        ...
        {isConnected && hasMintedQuery.data === true && address && (
          <div className="flex justify-center"><Link to={`/profile/${address}`}><Button>View your Builder Passport</Button></Link></div>
        )}
        {isConnected && hasMintedQuery.data === false && (
          <div className="flex justify-center"><Link to="/mint"><Button>Mint your Builder Passport</Button></Link></div>
        )}
      </section>
      ...
      <section className="rounded-md border border-line bg-ink p-6 text-paper">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-paper/70">
          BuilderPass does not manufacture a subjective score. The foundation is verifiable identity;
          future attestations can connect skills, projects, deployments and contributions to that identity.
        </p>
      </section>
      ...
      <section className="border-t border-line pt-8 text-left">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate">
          The long-term model is Identity → Credentials → Reputation → Builder Graph. That makes
          builder evidence machine-readable for ecosystems, applications and eventually autonomous agents,
          without turning the passport into an opaque scoring system.
        </p>
      </section>
    </div>
  );
